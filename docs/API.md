...
| `POST /api/student/upload/` (multipart) | Upload a document to one section | `section`, `file` or `event_report_url` (LUCS link mode) | `400` if > 150 KB or not JPG/PNG/PDF; re-opens a `REJECTED` section to `PENDING` on new upload; runs advisory OCR and returns `{warnings, autofill}` |
...
| Method + path | Auth |
|---|---|
| `GET /api/document/<id>/download/` | Owning student, the section's officer (in scope), or a consolidator (HOD within its department / Administration for any section). `403` otherwise, `404` if missing |
